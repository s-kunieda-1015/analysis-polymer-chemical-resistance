...
# Load polymer_mpk and pe_solvent datasets
...
plt.figure()
plt.hist(df_MPK_vPolymer["resistance_pred"], bins=100)
plt.tight_layout()
plt.close()
...
df_MPK_vPolymer["resistance_pred"]
df_MPK_vPolymer[features]
features = list(all_features_combinations[feature_No])
exclude_cols = ['n_atom_radonpy_polymer', 'mol_weight_radonpy_polymer']
features = [col for col in features if col not in exclude_cols]
train_data = df_MPK_vPolymer.copy()[list(features)]
X_train_exp = train_data[features]
CL_label = df_MPK_vPolymer["crystalinity_binary"]
